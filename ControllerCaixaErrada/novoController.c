...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    int sensores[6] = {LeituraSensorProx[0],LeituraSensorProx[1],LeituraSensorProx[6],LeituraSensorProx[7]};
    int timeout = 0;
    for (int i = 0; i < 6;++i){
       timeout = 0;
       if(sensores[i]>30){
        ...
        for(int i=0;i<QtddCaixa;i++){
          ...
          printf("CAIXA%02d %f   %f  %f\n\n",i+1,PosicaoCaixa[0],PosicaoCaixa[1],PosicaoCaixa[2]);
          for (int m = 0;m<3; ++m){
            if (listaPos[i][m] != PosicaoCaixa[m]){
              printf("lista:%f PosAtual:%f\n",listaPos[i][m],PosicaoCaixa[m]);
              //dancinha!
              printf("Mecheu!\n");
