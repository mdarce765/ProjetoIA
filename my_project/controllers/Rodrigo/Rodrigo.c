...
int main(int argc, char **argv) {
  ...
  //    Mudar o DEF da Wooden Box para CAIXA
   ...
   WbNodeRef puck = wb_supervisor_node_get_from_def("PUCK");
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    const double *PosicaoPuck = wb_supervisor_node_get_position(puck);
    
    sprintf(texto,"%s|puck em x=%5.2f, y=%5.2f, z=%5.2f",texto,PosicaoPuck[0],PosicaoPuck[1],PosicaoPuck[2]);
    const double *RotacaoPuck = wb_supervisor_field_get_sf_rotation(wb_supervisor_node_get_field(puck,"rotation"));
    
    double Rotacao = RotacaoPuck[3]*180/pi;
    sprintf(texto,"%s|rotacao = %5.2f",texto,Rotacao);
